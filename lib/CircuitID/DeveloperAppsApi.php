...
/**
     ...
     *
     * @param  mixed $id The ObjectId (unique 12 bytes ID) of record you would like to GET. (required)
     * @param  \OpenAPI\Client\CircuitID\DeveloperappsNotRequired $developerappsNotRequired The request data. (required)
     ...
     * @return mixed
     */
    public function patchDeveloperApp($id, $developerappsNotRequired, string $contentType = self::contentTypes['patchDeveloperApp'][0])
    {
        list($response) = $this->patchDeveloperAppWithHttpInfo($id, $developerappsNotRequired, $contentType);
        return $response;
    }
     ...
     * @return array of mixed, HTTP status code, HTTP response headers (array of strings)
     */
    public function patchDeveloperAppWithHttpInfo($id, $developerappsNotRequired, string $contentType = self::contentTypes['patchDeveloperApp'][0])
    {
        $request = $this->patchDeveloperAppRequest($id, $developerappsNotRequired, $contentType);
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function patchDeveloperAppAsync($id, $developerappsNotRequired, string $contentType = self::contentTypes['patchDeveloperApp'][0])
    {
        return $this->patchDeveloperAppAsyncWithHttpInfo($id, $developerappsNotRequired, $contentType)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function patchDeveloperAppAsyncWithHttpInfo($id, $developerappsNotRequired, string $contentType = self::contentTypes['patchDeveloperApp'][0])
    {
        $returnType = 'mixed';
        $request = $this->patchDeveloperAppRequest($id, $developerappsNotRequired, $contentType);
     ...
     * @return \GuzzleHttp\Psr7\Request
     */
    public function patchDeveloperAppRequest($id, $developerappsNotRequired, string $contentType = self::contentTypes['patchDeveloperApp'][0])
    {
        ...
        // verify the required parameter 'developerappsNotRequired' is set
        if ($developerappsNotRequired === null || (is_array($developerappsNotRequired) && count($developerappsNotRequired) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $developerappsNotRequired when calling patchDeveloperApp'
            );
        }
        ...
        // for model (json/xml)
        if (isset($developerappsNotRequired)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false) {
                # if Content-Type contains "application/json", json_encode the body
                $httpBody = \GuzzleHttp\Utils::jsonEncode(ObjectSerializer::sanitizeForSerialization($developerappsNotRequired));
            } else {
                $httpBody = $developerappsNotRequired;
            }
        } elseif (count($formParams) > 0) {
            if ($multipart) {
                $multipartContents = [];
