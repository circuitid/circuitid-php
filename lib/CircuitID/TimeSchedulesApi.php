...
/**
     ...
     *
     * @param  mixed $id The ObjectId (unique 12 bytes ID) of record you would like to GET. (required)
     * @param  \OpenAPI\Client\CircuitID\TimeschedulesNotRequired $timeschedulesNotRequired The request data. (required)
     ...
     * @return mixed
     */
    public function patchTimeSchedule($id, $timeschedulesNotRequired, string $contentType = self::contentTypes['patchTimeSchedule'][0])
    {
        list($response) = $this->patchTimeScheduleWithHttpInfo($id, $timeschedulesNotRequired, $contentType);
        return $response;
    }
     ...
     * @return array of mixed, HTTP status code, HTTP response headers (array of strings)
     */
    public function patchTimeScheduleWithHttpInfo($id, $timeschedulesNotRequired, string $contentType = self::contentTypes['patchTimeSchedule'][0])
    {
        $request = $this->patchTimeScheduleRequest($id, $timeschedulesNotRequired, $contentType);
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function patchTimeScheduleAsync($id, $timeschedulesNotRequired, string $contentType = self::contentTypes['patchTimeSchedule'][0])
    {
        return $this->patchTimeScheduleAsyncWithHttpInfo($id, $timeschedulesNotRequired, $contentType)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function patchTimeScheduleAsyncWithHttpInfo($id, $timeschedulesNotRequired, string $contentType = self::contentTypes['patchTimeSchedule'][0])
    {
        $returnType = 'mixed';
        $request = $this->patchTimeScheduleRequest($id, $timeschedulesNotRequired, $contentType);
     ...
     * @return \GuzzleHttp\Psr7\Request
     */
    public function patchTimeScheduleRequest($id, $timeschedulesNotRequired, string $contentType = self::contentTypes['patchTimeSchedule'][0])
    {
        ...
        // verify the required parameter 'timeschedulesNotRequired' is set
        if ($timeschedulesNotRequired === null || (is_array($timeschedulesNotRequired) && count($timeschedulesNotRequired) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $timeschedulesNotRequired when calling patchTimeSchedule'
            );
        }
        ...
        // for model (json/xml)
        if (isset($timeschedulesNotRequired)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false) {
                # if Content-Type contains "application/json", json_encode the body
                $httpBody = \GuzzleHttp\Utils::jsonEncode(ObjectSerializer::sanitizeForSerialization($timeschedulesNotRequired));
            } else {
                $httpBody = $timeschedulesNotRequired;
            }
        } elseif (count($formParams) > 0) {
            if ($multipart) {
                $multipartContents = [];
