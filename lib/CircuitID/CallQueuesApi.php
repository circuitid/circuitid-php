...
/**
     ...
     *
     * @param  mixed $id The ObjectId (unique 12 bytes ID) of record you would like to GET. (required)
     * @param  \OpenAPI\Client\CircuitID\CallqueuesNotRequired $callqueuesNotRequired The request data. (required)
     ...
     * @return mixed
     */
    public function patchCallQueue($id, $callqueuesNotRequired, string $contentType = self::contentTypes['patchCallQueue'][0])
    {
        list($response) = $this->patchCallQueueWithHttpInfo($id, $callqueuesNotRequired, $contentType);
        return $response;
    }
     ...
     * @return array of mixed, HTTP status code, HTTP response headers (array of strings)
     */
    public function patchCallQueueWithHttpInfo($id, $callqueuesNotRequired, string $contentType = self::contentTypes['patchCallQueue'][0])
    {
        $request = $this->patchCallQueueRequest($id, $callqueuesNotRequired, $contentType);
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function patchCallQueueAsync($id, $callqueuesNotRequired, string $contentType = self::contentTypes['patchCallQueue'][0])
    {
        return $this->patchCallQueueAsyncWithHttpInfo($id, $callqueuesNotRequired, $contentType)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function patchCallQueueAsyncWithHttpInfo($id, $callqueuesNotRequired, string $contentType = self::contentTypes['patchCallQueue'][0])
    {
        $returnType = 'mixed';
        $request = $this->patchCallQueueRequest($id, $callqueuesNotRequired, $contentType);
     ...
     * @return \GuzzleHttp\Psr7\Request
     */
    public function patchCallQueueRequest($id, $callqueuesNotRequired, string $contentType = self::contentTypes['patchCallQueue'][0])
    {
        ...
        // verify the required parameter 'callqueuesNotRequired' is set
        if ($callqueuesNotRequired === null || (is_array($callqueuesNotRequired) && count($callqueuesNotRequired) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $callqueuesNotRequired when calling patchCallQueue'
            );
        }
        ...
        // for model (json/xml)
        if (isset($callqueuesNotRequired)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false) {
                # if Content-Type contains "application/json", json_encode the body
                $httpBody = \GuzzleHttp\Utils::jsonEncode(ObjectSerializer::sanitizeForSerialization($callqueuesNotRequired));
            } else {
                $httpBody = $callqueuesNotRequired;
            }
        } elseif (count($formParams) > 0) {
            if ($multipart) {
                $multipartContents = [];
