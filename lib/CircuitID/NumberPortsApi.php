...
/**
     ...
     *
     * @param  mixed $id The ObjectId (unique 12 bytes ID) of record you would like to GET. (required)
     * @param  \OpenAPI\Client\CircuitID\NumberportsNotRequired $numberportsNotRequired The request data. (required)
     ...
     * @return mixed
     */
    public function patchNumberPort($id, $numberportsNotRequired, string $contentType = self::contentTypes['patchNumberPort'][0])
    {
        list($response) = $this->patchNumberPortWithHttpInfo($id, $numberportsNotRequired, $contentType);
        return $response;
    }
     ...
     * @return array of mixed, HTTP status code, HTTP response headers (array of strings)
     */
    public function patchNumberPortWithHttpInfo($id, $numberportsNotRequired, string $contentType = self::contentTypes['patchNumberPort'][0])
    {
        $request = $this->patchNumberPortRequest($id, $numberportsNotRequired, $contentType);
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function patchNumberPortAsync($id, $numberportsNotRequired, string $contentType = self::contentTypes['patchNumberPort'][0])
    {
        return $this->patchNumberPortAsyncWithHttpInfo($id, $numberportsNotRequired, $contentType)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function patchNumberPortAsyncWithHttpInfo($id, $numberportsNotRequired, string $contentType = self::contentTypes['patchNumberPort'][0])
    {
        $returnType = 'mixed';
        $request = $this->patchNumberPortRequest($id, $numberportsNotRequired, $contentType);
     ...
     * @return \GuzzleHttp\Psr7\Request
     */
    public function patchNumberPortRequest($id, $numberportsNotRequired, string $contentType = self::contentTypes['patchNumberPort'][0])
    {
        ...
        // verify the required parameter 'numberportsNotRequired' is set
        if ($numberportsNotRequired === null || (is_array($numberportsNotRequired) && count($numberportsNotRequired) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $numberportsNotRequired when calling patchNumberPort'
            );
        }
        ...
        // for model (json/xml)
        if (isset($numberportsNotRequired)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false) {
                # if Content-Type contains "application/json", json_encode the body
                $httpBody = \GuzzleHttp\Utils::jsonEncode(ObjectSerializer::sanitizeForSerialization($numberportsNotRequired));
            } else {
                $httpBody = $numberportsNotRequired;
            }
        } elseif (count($formParams) > 0) {
            if ($multipart) {
                $multipartContents = [];
