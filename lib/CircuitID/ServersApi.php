...
/**
     ...
     *
     * @param  mixed $id The ObjectId (unique 12 bytes ID) of record you would like to GET. (required)
     * @param  \OpenAPI\Client\CircuitID\ServersNotRequired $serversNotRequired The request data. (required)
     ...
     * @return mixed
     */
    public function patchServer($id, $serversNotRequired, string $contentType = self::contentTypes['patchServer'][0])
    {
        list($response) = $this->patchServerWithHttpInfo($id, $serversNotRequired, $contentType);
        return $response;
    }
     ...
     * @return array of mixed, HTTP status code, HTTP response headers (array of strings)
     */
    public function patchServerWithHttpInfo($id, $serversNotRequired, string $contentType = self::contentTypes['patchServer'][0])
    {
        $request = $this->patchServerRequest($id, $serversNotRequired, $contentType);
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function patchServerAsync($id, $serversNotRequired, string $contentType = self::contentTypes['patchServer'][0])
    {
        return $this->patchServerAsyncWithHttpInfo($id, $serversNotRequired, $contentType)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function patchServerAsyncWithHttpInfo($id, $serversNotRequired, string $contentType = self::contentTypes['patchServer'][0])
    {
        $returnType = 'mixed';
        $request = $this->patchServerRequest($id, $serversNotRequired, $contentType);
     ...
     * @return \GuzzleHttp\Psr7\Request
     */
    public function patchServerRequest($id, $serversNotRequired, string $contentType = self::contentTypes['patchServer'][0])
    {
        ...
        // verify the required parameter 'serversNotRequired' is set
        if ($serversNotRequired === null || (is_array($serversNotRequired) && count($serversNotRequired) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $serversNotRequired when calling patchServer'
            );
        }
        ...
        // for model (json/xml)
        if (isset($serversNotRequired)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false) {
                # if Content-Type contains "application/json", json_encode the body
                $httpBody = \GuzzleHttp\Utils::jsonEncode(ObjectSerializer::sanitizeForSerialization($serversNotRequired));
            } else {
                $httpBody = $serversNotRequired;
            }
        } elseif (count($formParams) > 0) {
            if ($multipart) {
                $multipartContents = [];
