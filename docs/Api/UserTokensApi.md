...
```php
createUserToken($usertokens): mixed
```
...
```php
findUserTokens($search, $limit, $skip, $sort, $select, $or, $and)
```
...
try {
    $apiInstance->findUserTokens($search, $limit, $skip, $sort, $select, $or, $and);
} catch (Exception $e) {
...
try {
    $result = $apiInstance->getUserToken($id);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling UserTokensApi->getUserToken: ', $e->getMessage(), PHP_EOL;
}
```
...
```php
patchUserToken($id, $usertokens): mixed
```
...
try {
    $result = $apiInstance->removeUserToken($id);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling UserTokensApi->removeUserToken: ', $e->getMessage(), PHP_EOL;
}
```
