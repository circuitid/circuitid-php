# OpenAPI\Client\AppMarketplaceApi
...
```php
findAppMarketplace($search, $limit, $skip, $sort, $select, $or, $and): \OpenAPI\Client\CircuitID\FindAppMarketplace200Response
```
...
$apiInstance = new OpenAPI\Client\Api\AppMarketplaceApi(
...
try {
    $result = $apiInstance->findAppMarketplace($search, $limit, $skip, $sort, $select, $or, $and);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling AppMarketplaceApi->findAppMarketplace: ', $e->getMessage(), PHP_EOL;
}
```
