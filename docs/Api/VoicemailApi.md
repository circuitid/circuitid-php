...
```php
findVoicemail($search, $limit, $skip, $sort, $select, $or, $and): \OpenAPI\Client\CircuitID\FindAppMarketplace200Response
```
...
try {
    $result = $apiInstance->findVoicemail($search, $limit, $skip, $sort, $select, $or, $and);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling VoicemailApi->findVoicemail: ', $e->getMessage(), PHP_EOL;
}
```
...
try {
    ...
    echo 'Exception when calling VoicemailApi->getVoicemail: ', $e->getMessage(), PHP_EOL;
}
```
