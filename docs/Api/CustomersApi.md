...
```php
createCustomer($customers): mixed
```
...
```php
findCustomers($search, $limit, $skip, $sort, $select, $or, $and)
```
...
try {
    $apiInstance->findCustomers($search, $limit, $skip, $sort, $select, $or, $and);
} catch (Exception $e) {
...
```php
patchCustomer($id, $customers): mixed
```
