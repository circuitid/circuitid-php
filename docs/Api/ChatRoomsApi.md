...
```php
createChatRoom($chatrooms): mixed
```
...
```php
findChatRooms($search, $limit, $skip, $sort, $select, $or, $and)
```
...
try {
    $apiInstance->findChatRooms($search, $limit, $skip, $sort, $select, $or, $and);
} catch (Exception $e) {
...
```php
patchChatRoom($id, $chatrooms): mixed
```
