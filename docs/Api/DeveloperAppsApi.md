...
```php
createDeveloperApp($developerapps): mixed
```
...
```php
findDeveloperApps($search, $limit, $skip, $sort, $select, $or, $and)
```
...
try {
    $apiInstance->findDeveloperApps($search, $limit, $skip, $sort, $select, $or, $and);
} catch (Exception $e) {
...
```php
patchDeveloperApp($id, $developerapps): mixed
```
