...
```php
findFindNumbers($search, $limit, $skip, $sort, $select, $or, $and)
```
...
$apiInstance = new OpenAPI\Client\Api\FindNumbersApi(
...
try {
    $apiInstance->findFindNumbers($search, $limit, $skip, $sort, $select, $or, $and);
} catch (Exception $e) {
