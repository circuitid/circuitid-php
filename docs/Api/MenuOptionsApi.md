...
```php
findMenuOptions($search, $limit, $skip, $sort, $select, $or, $and)
```
...
try {
    $apiInstance->findMenuOptions($search, $limit, $skip, $sort, $select, $or, $and);
} catch (Exception $e) {
...
```php
patchMenuOption($id, $menuoptions): mixed
```
