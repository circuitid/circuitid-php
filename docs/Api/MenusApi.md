...
```php
findMenus($search, $limit, $skip, $sort, $select, $or, $and)
```
...
try {
    $apiInstance->findMenus($search, $limit, $skip, $sort, $select, $or, $and);
} catch (Exception $e) {
