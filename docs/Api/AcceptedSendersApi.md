# OpenAPI\Client\AcceptedSendersApi
...
```php
createAcceptedSender($acceptedsenders): mixed
```
...
```php
findAcceptedSenders($search, $limit, $skip, $sort, $select, $or, $and)
```
...
try {
    $apiInstance->findAcceptedSenders($search, $limit, $skip, $sort, $select, $or, $and);
} catch (Exception $e) {
...
try {
    $result = $apiInstance->getAcceptedSender($id);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling AcceptedSendersApi->getAcceptedSender: ', $e->getMessage(), PHP_EOL;
}
```
...
```php
patchAcceptedSender($id, $acceptedsenders): mixed
```
...
try {
    $result = $apiInstance->removeAcceptedSender($id);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling AcceptedSendersApi->removeAcceptedSender: ', $e->getMessage(), PHP_EOL;
}
```
