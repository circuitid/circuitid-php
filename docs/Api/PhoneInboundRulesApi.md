...
```php
findPhoneInboundRules($search, $limit, $skip, $sort, $select, $or, $and)
```
...
try {
    $apiInstance->findPhoneInboundRules($search, $limit, $skip, $sort, $select, $or, $and);
} catch (Exception $e) {
...
```php
patchPhoneInboundRule($id, $phoneinboundrules): mixed
```
